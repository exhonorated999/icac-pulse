...
# Creates NSIS installer with security features
...
Write-Host "  Building: NSIS, MSI, and Portable versions" -ForegroundColor Gray
Write-Host ""
...
if ($LASTEXITCODE -ne 0) {
    Write-Host ""
    Write-Host "X Installer creation failed!" -ForegroundColor Red
    Write-Host "  Check the error messages above" -ForegroundColor Yellow
    pause
    exit 1
}
...
if (Test-Path "dist") {
    ...
    if ($files) {
        ...
        Write-Host "  5. Distribute to users" -ForegroundColor White
        Write-Host ""
        
        Write-Host "Recommended installer:" -ForegroundColor Yellow
        Write-Host "  ICAC P.U.L.S.E.-*-Setup.exe (NSIS)" -ForegroundColor Cyan
        Write-Host ""
